...
function startLanguageServer(context: vscode.ExtensionContext) {
	...
	if (!serverPath) {
		const errorMsg = 'QuickMark server binary not found. Please install quickmark_server or configure the server path.';
	...
	client.start().then(() => {
		...
		vscode.window.showErrorMessage(
			`Failed to start QuickMark language server. Please ensure quickmark_server is installed and accessible. Error: ${error.message}`
		);
	});
